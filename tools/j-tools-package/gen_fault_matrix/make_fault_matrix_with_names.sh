#!/bin/sh
if test $# -lt 8
then echo "Usage: ${0} <subject name> <subject installation script> <version start> <number of versions> <universe path> <test universe file> <executable file> <fault matrix file> [<line_adjust>]"
     exit 1
else
     if test $# -gt 9
     then echo "Usage: ${0} <subject name> <subject installation script> <version start> <number of versions> <universe path> <test universe file> <executable file> <fault matrix file> [<line_adjust>]"
